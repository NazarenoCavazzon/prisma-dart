...
class _OutputObjectTypesBuilder {
  ...
  void _buildOutputTypes(List<dmmf.OutputType> outputTypes) {
    for (final dmmf.OutputType type in outputTypes) {
      ...
      library.body.add(
        code_builder.Class(
          (code_builder.ClassBuilder builder) {
                  ...
                  builder.requiredParameters.add(
                    code_builder.Parameter(
                      (code_builder.ParameterBuilder builder) {
                        builder.name = 'json';
                        builder.type =
                            code_builder.Reference('Map<String, dynamic>');
                      },
                    ),
                  );
  ...
  void _constructorBuilder(
    List<dmmf.SchemaField> fields,
    code_builder.ClassBuilder classBuilder,
  ) {
  ...
  void _fieldsBuilder(
    List<dmmf.SchemaField> fields,
    code_builder.ClassBuilder updates,
  ) {
    for (final dmmf.SchemaField field in fields) {
      ...
      updates.fields.add(
        code_builder.Field(
          (code_builder.FieldBuilder updates) {
...
class _InputObjectTypesBuilder {
  ...
  void _toJsonBuilder(code_builder.ClassBuilder updates, dmmf.InputType input) {
    updates.methods.add(
      code_builder.Method(
        (code_builder.MethodBuilder updates) {
          updates.annotations.add(
            code_builder.Reference('override'),
          );
          updates.name = 'toJson';
          ...
          updates.body = code_builder.Block(
            (code_builder.BlockBuilder updates) {
              updates.statements
                  .add(code_builder.Code('return <String, dynamic>'));
        ...
        code_builder.Field(
          (code_builder.FieldBuilder updates) {
            ...
            updates.type = schemaTypeResolver(field.inputTypes,
                field.isNullable == true || !field.isRequired);
